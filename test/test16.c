...
int (*clever1(int a, int b, int (*a_fptr)(int, int), int(*b_fptr)(int, int) ))(int, int) {
   return clever(a,b,b_fptr,a_fptr);
}
int moo(char x, int op1, int op2) {
    int (*a_fptr)(int, int) = plus;
    int (*s_fptr)(int, int) = minus;
    int (* (*goo_ptr)(int, int, int (*)(int, int), int(*)(int, int)))(int,int)=clever1;

    int (*t_fptr)(int,int) = goo_ptr(op1, op2, s_fptr, a_fptr); 
    t_fptr(op1, op2);
